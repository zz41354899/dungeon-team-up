<template>
  <div class="min-h-screen bg-black text-white relative overflow-hidden">
    ...
    <!-- 固定導覽列 -->
    <nav class="fixed top-6 left-6 right-6 z-50 flex items-center justify-between">
      <!-- Logo -->
      ...
      <div class="flex items-center space-x-3">
        <NuxtLink 
          v-if="user"
          to="/dashboard"
          class="px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-xl transition-colors text-sm font-medium text-blue-300"
        >
          控制面板
        </NuxtLink>
        <NuxtLink 
          v-else
          to="/login"
          class="border border-white/20 hover:border-white/40 text-white px-6 py-3 rounded-xl transition-colors"
        >
          開始冒險
        </NuxtLink>
      </div>
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center">
      <div class="max-w-screen-lg mx-auto px-4 sm:px-8 text-center animate-fade-in">
        
        <!-- Beta 標籤 -->
        <div class="inline-flex items-center px-4 py-2 bg-cyan-500/10 border border-cyan-500/20 rounded-full text-sm text-cyan-300 mb-8">
        ...
        <!-- 副標題 -->
        <p class="text-lg sm:text-xl text-zinc-300/90 mb-4 max-w-3xl mx-auto leading-relaxed tracking-wider">
          一起完成副本任務
        </p>
        
        <p class="text-base text-zinc-400 max-w-2xl mx-auto mb-12">
        ...
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
          <NuxtLink 
            to="/login"
            class="bg-blue-600 hover:bg-blue-700 transition-colors text-white font-medium px-6 py-3 rounded-xl shadow-xl flex items-center space-x-3"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
            </svg>
            <span class="text-lg">立即登入</span>
          </NuxtLink>
          
          <button 
            @click="openSearchModal"
            class="border border-white/20 hover:border-white/40 text-white px-6 py-3 rounded-xl transition-colors flex items-center space-x-3"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-3 gap-8 max-w-md mx-auto">
          <div class="flex flex-col items-center space-y-3">
            ...
            <span class="text-sm text-gray-400">組隊</span>
          </div>
          ...
          <div class="flex flex-col items-center space-y-3">
            ...
            <span class="text-sm text-gray-400">任務</span>
          </div>
          ...
          <div class="flex flex-col items-center space-y-3">
            ...
            <span class="text-sm text-gray-400">冒險</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showSearchModal" class="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div class="relative max-w-2xl w-full mx-4 bg-gray-900/90 border border-white/10 rounded-2xl shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-2xl font-bold text-white">搜尋任務</h3>
          <button 
        ...
        <div class="mb-6">
          <div class="relative">
            <input 
              v-model="searchQuery"
              type="text" 
              class="w-full bg-black/30 border border-white/20 rounded-xl px-4 py-3 pl-12 text-white placeholder-gray-500 focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
              placeholder="輸入任務關鍵字..."
        ...
        <div class="max-h-96 overflow-y-auto">
          <div v-if="searchLoading" class="text-center py-8">
            ...
            <p class="text-gray-400 mt-4">搜尋中...</p>
          </div>
          <div v-else-if="searchResults.length === 0 && searchQuery" class="text-center py-8">
            <p class="text-gray-400">沒有找到相關任務</p>
          </div>
          <div v-else-if="searchResults.length === 0" class="text-center py-8">
            <p class="text-gray-400">輸入關鍵字開始搜尋</p>
          </div>
          <div v-else class="space-y-3">
            <div 
              v-for="mission in searchResults" 
              :key="mission.id"
              class="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/10 transition-colors"
            >
              <div class="flex items-start justify-between">
                <div>
                  <h4 class="text-lg font-semibold text-white mb-2">{{ mission.title }}</h4>
                  <p class="text-sm text-gray-400 mb-2">{{ mission.description || '無說明' }}</p>
                  <div class="flex items-center space-x-2">
                    <span class="px-2 py-1 bg-emerald-500/20 rounded-lg text-xs text-emerald-300">
                      {{ mission.projects?.title || '未知專案' }}
                    </span>
    ...
    <div v-if="showProfilesModal" class="fixed inset-0 z-[9999] flex items-center justify-center">
      ...
      <div class="relative max-w-4xl w-full mx-4 bg-gray-900/90 border border-white/10 rounded-2xl shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
          <h3 class="text-2xl font-bold text-white">冒險者角色卡</h3>
          <button 
            ...
            >
              <div class="flex items-center space-x-3 mb-3">
                <img 
                  ...
                  class="w-12 h-12 rounded-full border-2 border-purple-400/50"
                >
                <div v-else class="w-12 h-12 bg-gradient-to-br from-purple-500 to-blue-500 rounded-full flex items-center justify-center border-2 border-purple-400/50">
                  <span class="text-white text-lg font-bold">
                    {{ profile.username.charAt(0).toUpperCase() }}
                  </span>
                </div>
                <div>
                  <h4 class="text-lg font-semibold text-white">{{ profile.username }}</h4>
                  <div class="px-2 py-1 bg-purple-500/20 rounded-lg text-xs text-purple-300">
                    {{ profile.style || '未設定' }}
                  </div>
                </div>
              </div>
...
const searchMissions = async () => {
  ...
  try {
    ...
    const { data, error } = await $supabase
      .from('missions')
      .select('id, title, description, project_id, created_by, created_at, projects(title)')
      .ilike('title', `%${searchQuery.value.trim()}%`)
    ...
    // 處理 Supabase join 結果
    searchResults.value = (data || []).map((mission) => ({
      ...mission,
      projects: mission.projects && mission.projects.length > 0 ? mission.projects[0] : null
    }))
  } catch (error) {
